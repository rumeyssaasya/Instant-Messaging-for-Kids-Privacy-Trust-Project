...
def open_register_screen(root):
    ...
    # Kayıt Ol butonu
    tk.Button(register_root, text="Kayıt Ol", command=register).pack(pady=5)

    # Geri butonu
def go_back():
    register_root.destroy()  # Giriş ekranını kapat
    main_root.deiconify()  # Ana ekranı göster

tk.Button(register_root, text="Geri", command=go_back).pack(pady=5)
