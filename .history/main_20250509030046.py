...
class KidMessenger:
        ...
        try:
            if selected_contact.startswith("👶"):
                ...
                query = """SELECT u.username, m.message, m.timestamp, m.approved 
                        ...
                        AND (m.visible_to_child=1 OR ?=1)  -- Ebeveynler tüm mesajları görür
    ...
    def review_messages(self):
            # Close any existing review window first
        for widget in self.root.winfo_children():
            if isinstance(widget, tk.Toplevel) and widget.title() == "Review Messages":
                widget.destroy()
        
        # Rest of the existing review_messages code...
        ...
        for msg in pending_messages:
            ...
            # Include message ID in the display
    ...
    def process_message_approval(self, message_id, approved):
        try:
            ...
            # If rejected, set visible_to_child to 0
            if not approved:
                try:
            ...
            # Destroy the review window before showing success message
            for widget in self.root.winfo_children():
                if isinstance(widget, tk.Toplevel) and widget.title() == "Review Messages":
                    widget.destroy()
            ...
            # Only reopen review window if there might be more messages to review
            if approved:
