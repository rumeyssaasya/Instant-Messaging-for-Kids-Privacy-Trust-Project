...
class KidMessenger:
    ...
    def process_message_approval(self, message_id, approved):
        try:
            # Önce mesajın onay durumunu güncelle
            self.c.execute("UPDATE messages SET approved=? WHERE id=?", (approved, message_id))
            
            # Eğer reddedildiyse (approved=0), visible_to_child'ı 0 yap
            if not approved:
                # Önce sütunun var olduğundan emin ol
                try:
                    self.c.execute("UPDATE messages SET visible_to_child=0 WHERE id=?", (message_id,))
                except sqlite3.OperationalError:
                    # Sütun yoksa oluştur ve tekrar dene
                    self.update_database_schema()
                    self.c.execute("UPDATE messages SET visible_to_child=0 WHERE id=?", (message_id,))
            ...
            self.review_messages()  # Mesaj listesini yenile
