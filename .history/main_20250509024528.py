...
class KidMessenger:
    ...
    def load_messages(self, event=None):
        ...
        try:
            if selected_contact.startswith("👶"):
                ...
                self.c.execute(query, (child_id, child_id))
            else:
                # Regular chat
                ...
                query = """SELECT u.username, m.message, m.timestamp, m.approved 
                        ...
                        ORDER BY m.timestamp"""
                self.c.execute(query, (self.current_user[0], contact_id, contact_id, self.current_user[0]))
            ...
            for message in messages:
                ...
                # Only show approved messages to children
                if not self.is_parent and not approved:
                    continue
                    
                display_msg = f"{username} ({timestamp}): {msg}\n"
                
                if self.is_parent and not approved:
    ...
    def check_contact_approval(self):
        selection = self.contacts_listbox.curselection()
    ...
    def process_message_approval(self, message_id, approved):
        ...
        # Reddedilen mesajları çocukların görmemesi için ekstra önlem
        if not approved:
            # Mesajın alıcısını ve göndericisini al
            self.c.execute("SELECT sender_id, receiver_id FROM messages WHERE id=?", (message_id,))
            sender_id, receiver_id = self.c.fetchone()
            
            # İlgili çocuk hesaplarını bul (hem gönderen hem alıcı için)
            child_ids = []
            self.c.execute("SELECT id FROM users WHERE parent_id=?", (self.current_user[0],))
            for row in self.c.fetchall():
                child_ids.append(row[0])
            
            # Eğer mesaj çocuklardan birine aitse, reddedildi olarak işaretle
            if sender_id in child_ids or receiver_id in child_ids:
                self.c.execute("UPDATE messages SET visible_to_child=0 WHERE id=?", (message_id,))
        
        self.conn.commit()
    
        # Veritabanına yeni bir sütun eklememiz gerekiyor (ALTER TABLE)
        try:
    ...
    def update_database_schema(self):
        """Gerekli sütunları veritabanına ekler"""
        try:
            # visible_to_child sütunu ekle
            self.c.execute("ALTER TABLE messages ADD COLUMN visible_to_child INTEGER DEFAULT 1")
            
            # approved sütunu yoksa ekle
            self.c.execute("SELECT approved FROM messages LIMIT 1")
        except sqlite3.OperationalError:
            try:
                self.c.execute("ALTER TABLE messages ADD COLUMN approved INTEGER DEFAULT 0")
            except sqlite3.OperationalError:
